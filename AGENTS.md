...
An idea is a skill that makes a kind of document, and the website's Discover section is built from them. It is an ordinary skill folder with three additions: an `idea.json` sidecar (title, tagline, when to use it, tags, order, needs, which example is the cover, and optionally the sections a page contains), a `starter.html` the agent copies, and an `examples/` folder holding at least three finished pages that differ on purpose, each with a `.json` design note saying what that example chose and why, and a `variant`: one subject-free line under 80 characters saying what the example shows of the shape, which the website uses as the example's caption and viewer title with the subject title under it. Skills without a sidecar are not ideas and never appear on Discover.

- `idea.json` may carry a `sketch`: up to seven rows that the website draws as a small abstract page on the idea's tile, so a visitor sees the shape before any example. Rows: `k` kicker, `t` title, `T` big title, `p` paragraph, `s` full-width strip, `c2` `c3` `c4` a row of cards, `l` ledger rows, `m` a matrix, `g` a grade badge with lines, `d` a diagram of boxes and an arrow, `b` horizontal bars, `v` a vertical spine with stops, `q` a quoted block with a left rule, `x` checklist rows, `n` a numbered step, `2` two columns of text. Pick the six that say what the page is.
...
- Examples are real, not invented. They name things that exist, link each one to the page a reader can check it against, and carry only figures a source actually gave; where a page had to estimate, its footer says which figures those are. Real things go stale, so an example that names something superseded says so rather than quietly pointing at the replacement, and a real run of the skill that turns out better than an example should replace it.
