...
A template whose pages are drawn rather than written also ships a `scripts/` folder: the kit that writes the drawing, as a library the agent imports, in a `.mjs` and a `.py` that take the same calls and produce the same output. Embedding that kit in `patterns.md` for the agent to retype was the earlier shape and the wrong one; `patterns.md` names the files and shows the worked example. Both run with no dependency and no install, and running one directly writes a sample so an agent can check its environment before writing a page. Name the scripts in the template's own `template.md` too: the runtime hands an agent at most fifty filenames, so a file nothing names is a file nothing opens.
