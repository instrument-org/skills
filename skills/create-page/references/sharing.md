...
node <skill>/share.mjs output/<slug>.html --delete        # take the newest link down
node <skill>/share.mjs output/<slug>.html --delete <id>   # take an earlier one down
```
...
```sh
curl -sS -H 'content-type: text/html' --data-binary @output/<slug>.html -o output/<slug>.share.json https://share.instrument.page/share
```
