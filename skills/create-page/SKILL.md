...
4. Copy `starter.html` to `output/<slug>.html`, set its `instrument:idea` meta to the template's name and its `description` meta to one sentence saying what the page holds, and paste the template's `main.html` inside `<main>`. The description is what a shared link shows under the title; write it for someone deciding whether to open the page. The starter carries the tab icon, the skin, the fonts, the icon set, and the page behavior, each inside a `shell:start` … `shell:end` pair; leave those regions as they are, and everything under `<main>` is yours. Your own CSS goes in the gap the starter leaves between them.
