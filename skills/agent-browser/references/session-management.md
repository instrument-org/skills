...
When an origin does push back, do not answer it by fetching the same URLs another way. This is the trap that looks most like resourcefulness and is not. Copying a browser's headers onto a scripted client does not make it pass. Measured against one such site across roughly thirty single requests: a Node fetch, `curl` over either HTTP version, and two Python interpreters were all refused while carrying byte-identical copies of the headers a real browser sends, with the occasional success too erratic to steer by. Retrying does not fix it and neither does waiting. It also abandons the cookies and session that made the earlier requests legitimate. Do not read the status code as advice either: a refusal on shape is often served as `429`, which reads as a rate limit and is not one. Staying in the browser is the better bet and not a guarantee: a real browser gets refused by these sites too, minutes either side of a scripted client getting through. When that happens there is nothing left to try, so tell the user the site is refusing and offer what you can source another way rather than working down a list of clients.
