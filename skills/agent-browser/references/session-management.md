...
When an origin does push back, do not answer it by fetching the same URLs another way. This is the trap that looks most like resourcefulness and is not. A scripted HTTP client sends a bare user agent and none of the headers a browser sends, and that shape alone is enough for many sites to refuse it on the first request, whatever the URL and however long you wait -- measured, a request carrying only `User-Agent: Mozilla/5.0` is refused where the identical request with a browser's headers succeeds. It also abandons the cookies and session that made the earlier requests legitimate. Do not read the status code as advice either: a refusal on shape is often served as `429`, which reads as a rate limit and is not one. Stay in the browser, or tell the user the site is refusing and offer what you can source another way.
