...
A script rather than a documented `curl` line, because the bookkeeping is the part an agent drops: the token comes back once and is worthless in a transcript. The script writes it beside the page, where a later run or a later agent finds it, and turns the endpoint's status codes into sentences. It ships in both runtimes, `share.mjs` and `share.py`, the way the template kits do: the operation is one request, so the language of the task does not matter, but the runtime a harness has does, and a Python twin with no dependency costs eighty lines. Each reads the sidecar the other wrote. Both sit at the skill root beside `starter.html` rather than in a `scripts/` folder, which `check:skill` reads as a package wanting a `package.json`, and a `pyproject.toml` for the `.py`; these are files the agent reaches for directly, and the root is the first thing the runtime's file listing shows. `curl` is in the reference for a runtime with neither.
...
Two links can serve two versions of one page for up to thirty days after an edit, on purpose. The sidecar is one more file beside the output that carries a credential, low value but real: it can take down that one link and nothing else. A harness with neither Node nor Python falls back to `curl` and keeps the response by hand.
